//! Testmo API error types.
//!
//! Domain-specific error types for Testmo API operations.
...
mod tests {
    ...
    fn test_unauthorized_not_retryable() {
        let err = TestmoError::Unauthorized;
        assert!(!err.is_retryable());
    }

    #[test]
    fn test_not_found_not_retryable() {
        let err = TestmoError::NotFound("/projects/123".to_string());
        assert!(!err.is_retryable());
    }

    #[test]
    fn test_rate_limited_is_retryable() {
        let err = TestmoError::RateLimited;
        assert!(err.is_retryable());
    }

    #[test]
    fn test_server_error_is_retryable() {
        let err = TestmoError::ApiError {
